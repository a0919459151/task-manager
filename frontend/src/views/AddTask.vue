    ...
    <div class="bg-white rounded-lg shadow-xl p-8">
      ...
      <form @submit.prevent="saveTask">
        <div class="mb-6">
          ...
          <input type="text" id="title" v-model="newTask.title" placeholder="e.g. Design the new landing page"
            class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            required>
        </div>
        <div class="mb-6">
          <label for="content" class="block text-gray-700 font-semibold mb-2">Task Content</label>
          <textarea id="content" v-model="newTask.content" rows="4"
...
async function saveTask() {
  try {
    const response = await client.createTask({ body: newTask.value });
